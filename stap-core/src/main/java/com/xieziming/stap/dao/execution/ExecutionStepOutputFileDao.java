...
public class ExecutionStepOutputFileDao {
    ...
    public void add(ExecutionStepOutputFile executionStepOutputFile) {
        String sql = "INSERT INTO "+StapDbTables.EXECUTION_OUTPUT_FILE.toString()+" SET Execution_Step_Id=?, `Type`=?, File_Id, Remark";
        StapDbUtil.getJdbcTemplate().update(sql, new Object[]{executionStepOutputFile.getRawExecutionStep().getId(), executionStepOutputFile.getType(), executionStepOutputFile.getStapFile().getId(), executionStepOutputFile.getRemark()});
    }

    public void update(ExecutionStepOutputFile executionStepOutputFile) {
        String sql = "UPDATE "+StapDbTables.EXECUTION_OUTPUT_FILE.toString()+" SET Execution_Step_Id=?, `Type`=?, File_Id, Remark WHERE Id=?";
        StapDbUtil.getJdbcTemplate().update(sql, new Object[]{executionStepOutputFile.getRawExecutionStep().getId(), executionStepOutputFile.getType(), executionStepOutputFile.getStapFile().getId(), executionStepOutputFile.getRemark(), executionStepOutputFile.getId()});
    }

    public void delete(ExecutionStepOutputFile executionStepOutputFile) {
        String sql = "DELETE FROM "+StapDbTables.EXECUTION_OUTPUT_FILE.toString()+" WHERE Id=?";
    ...
    public ExecutionStepOutputFile findById(int id) {
        String sql = "SELECT * FROM " + StapDbTables.EXECUTION_OUTPUT_FILE.toString() + " WHERE Id=?";
        return  StapDbUtil.getJdbcTemplate().queryForObject(sql, new Object[]{id}, new RowMapper<ExecutionStepOutputFile>() {
            public ExecutionStepOutputFile mapRow(ResultSet resultSet, int i) throws SQLException {
                ...
                executionStepOutputFile.setRawExecutionStep(executionStepDao.findBasicById(resultSet.getInt("Execution_Step_Id")));
